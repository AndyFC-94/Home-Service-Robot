...
private:
  ...
  double init[2] = {2.334, -5.169};
  double end[2] = {-0.6048, -4.5569};
  double delta;
  const int mul= 2;
...
public:
  ...
  void odomCallback(const nav_msgs::Odometry::ConstPtr &msg) {
    ...
    if ((x_pose < (init[0] + x_scale)) && (x_pose > (init[0] - x_scale)) && (y_pose < (init[1] + y_scale)) && (y_pose > (init[1] -y_scale))) {
      ...
      else if ((x_pose < (end[0]+ 0.6)) && (x_pose > (end[0] - 0.6)) && (y_pose < (end[1] + 0.6))) {
